...
import com.june.generator.util.FileHelper;
import com.june.generator.util.IOHelper;
import com.june.generator.util.StringTemplate;
...
//	private static final String WEBAPP_GENERATOR_INSERT_AFTER_LOCATION = WEBAPP_GENERATOR_INSERT_LOCATION+":after";
	...
	public Generator(String driver,String url,String username,String password) {
		this.driver = driver;
		this.url = url;
	...
	public void generateAllTable() throws Exception {
		DbModelProvider provider = new DbModelProvider(driver, url, username, password);
	...
	private void generateTable(Table table) throws IOException, IllegalAccessException, InvocationTargetException, NoSuchMethodException, TemplateException {
		...
		URL furl = classLoader.getResource("custom");
		File templateRootDir = new File(furl.getFile()).getAbsoluteFile();
	...
	private Map getTemplateDataModel(Table table) {
		...
		model.put("basepackage", "com.june");
	...
	private String getOutRootDir() {
		ClassLoader classLoader = getClass().getClassLoader();
		URL furl = classLoader.getResource("outroot");
		return furl.getFile();
	}
